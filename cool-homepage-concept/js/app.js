$(window).bind('load', function () {
    ...
    $.event.special.swipe.scrollSupressionThreshold = (screen.availWidth);
    ...
    var app = {
        ...
        init: function () {
            ...
            this.reelEntryActions.swipeleft('swipeleft');
            this.reelEntryActions.swiperight('swiperight');
            this.reelEntryActions.touchstart('vmouseover');
            this.reelEntryActions.touchend('vmouseout');
            this.reelEntryActions.click('vclick');
        },
        cacheDOM: function () {
            ...
            this.actualWindow = $('#actual-window');
            this.actualWindow = $('#actual-window');
            this.actualFromTopOld;
            this.fromTopNew;
            ...
            this.diag = $('#diag');
            this.diagText = $('.diag-text');
            this.diagDirection = $('#diag-direction');
            this.diagCounter = $('#diag-counter');
            this.diagPosition = $('#diag-position');
            this.diagX = $('#diag-x');
            ...
            this.sectionInnerContent = $('.section-inner-content');
            this.aboutSectionInner = $('#about-section-inner');
            this.resumeSectionInner = $('#resume-section-inner');
            this.resumeSectionInnerContent = $('#resume-section-inner-content');
            this.resumeSectionInnerContentPadding = $('#resume-section-inner-content-padding');

            this.sectionInnerReel = $('.section-inner-reel');
            this.sectionInnerReelPosition = 0;
            this.sectionInnerReelDistanceFromOrigin = 0;
            this.sectionInnerReelEntry = $('.section-inner-reel-entry');
            this.sectionInnerReelEntryTapRegion = $('.entry-tap-region');
            this.reelEntryPositions = [];
            this.reelEntryTriggered = [];
            this.currentSectionInnerReelEntry;

            this.reelDragDetails = {
                currentDragDirectionTemp: null,
        ...
        entryChecker: function () {
            //            for (var i = 0; i < app.sectionInnerReelEntry.length; i++) {
            //                function entryReturn() {
            //                    app.reelEntryTriggered.map(function (item, itemI) {
            //                        if (itemI != 0) {
            //                            app.reelEntryTriggered[itemI] = false;
            //                        }
            //                    });
            //                }
            //                if (i < app.sectionInnerReelEntry.length - 1) {
            //                    if (app.sectionInnerReelDistanceFromOrigin >= app.reelEntryPositions[i].x && app.sectionInnerReelDistanceFromOrigin < app.reelEntryPositions[i + 1].x && !app.reelEntryTriggered[i]) {
            //                            app.reelEntryTriggered[i] = true;
            //                            console.log(`Triggered entry ${app.reelEntryPositions[i].id}`);
            //                            entryReturn();
            //                    }
            //                } else {
            //                    if (app.sectionInnerReelDistanceFromOrigin >= app.reelEntryPositions[i].x) {
            //                        entryReturn();
            //                    }
            //                }
            //            }
        ...
        RAFEvents: function () {
            ...
            if (app.aboutFromTop != null) {
                ...
                app.sectionInnerReelPosition = app.sectionInnerReel.offset().left;
                if (!app.swipeInProgress) {
                    app.sectionInnerReelDistanceFromOrigin = Math.abs(app.sectionInnerReelPosition);
                    app.diagPosition.html(`Distance From Origin: ${app.sectionInnerReelDistanceFromOrigin}`);
                }
        ...
        reelEntryActions: {
            swipeleft: function (touchType) {
                app.sectionInnerReelEntryTapRegion.on(touchType, function () {
                    app.reelDragDetails.counterNew++;
                    app.reelDragDetails.currentDragDirectionTemp = 'left';
                });
            },
            swiperight: function (touchType) {
                app.sectionInnerReelEntryTapRegion.on(touchType, function () {
                    app.reelDragDetails.counterNew++;
                    app.reelDragDetails.currentDragDirectionTemp = 'right';
                });
            },
            touchstart: function (touchType) {
                app.sectionInnerReelEntryTapRegion.on(touchType, function () {
                    app.reelDragDetails.isDragging = true;
                    if (!app.swipeInProgress) {
                        app.reelDragDetails.currentEntry = parseInt($(this).parent('.section-inner-reel-entry').attr('id'));
                    }
                });
            },
            touchend: function (touchType) {
                app.sectionInnerReelEntryTapRegion.on(touchType, function () {
                    app.reelDragDetails.currentDragDirection = app.reelDragDetails.currentDragDirectionTemp;
                    app.reelDragDetails.isDragging = false;
                });
            },
            click: function (touchType) {
                app.sectionInnerReelEntryTapRegion.on(touchType, function () {
        ...
        style: function () {
            this.cacheImage.imagesLoaded(function () {
                ...
                TweenMax.set(app.resumeSectionInnerContentPadding, {
                    height: app.skillBarsContainer.outerHeight() * 0.65
                });
                TweenMax.set(app.actualWindow, {
                    height: window.innerHeight
                });
                app.actualFromTopOld = app.fromTopNew;
                app.fromTopNew = app.actualWindow.offset().top;
                ...
                TweenMax.set(app.sectionInnerContent, {
                    perspective: 800,
                    perspectiveOrigin: 'center',
                    transformStyle: '2d'
                });
                ...
                TweenMax.set(app.skillBars, {
                    top: (app.skillBarsContainer.height() / 2 - app.skillBars.height() / 2) - window.innerHeight * 0.1,
                ...
                TweenMax.set(app.sectionInnerReel, {
                    width: app.sectionInnerReelEntry.width() * app.sectionInnerReelEntry.length,
                    height: app.sectionInnerReelEntry.eq(0).outerHeight()
                });

                TweenMax.set(app.sectionInnerReelEntryTapRegion, {
                    height: app.sectionInnerReel.outerHeight()
                });

                for (app.sirei = 0; app.sirei < app.sectionInnerReelEntry.length; app.sirei++) {
                    app.currentSectionInnerReelEntry = app.sectionInnerReelEntry.eq(app.sirei);
                    app.currentSectionInnerReelEntry.attr('id', app.sirei);
                    app.reelEntryPositions.push({
                        x: `-${parseInt(app.currentSectionInnerReelEntry.offset().left)}`,
                        id: app.currentSectionInnerReelEntry.attr('id')
                    });
                    app.reelEntryTriggered.push(false);
                }

                TweenMax.set(app.aboutSectionInner, {
                    height: app.aboutSectionInner.find('.section-inner-reel').outerHeight()
                });

                TweenMax.set(app.diag, {
        ...
        swipeChecker: function () {
            if (app.reelDragDetails.counterOld != app.reelDragDetails.counterNew) {

                //WHEN FINGER RELEASES FROM DRAGGING...
                if (!app.reelDragDetails.isDragging) {
                    app.swipeInProgress = true;

                    app.reelDragDetails.counterOld = app.reelDragDetails.counterNew;
                    app.diagDirection.html(`swiped ${app.reelDragDetails.currentDragDirection}!`);
                    app.diagCounter.html(`Counter: ${app.reelDragDetails.counterNew}`);
                    if (app.reelDragDetails.currentDragDirection === 'left') {
                        blockMenuHeaderScroll = true;
                        if (app.reelDragDetails.currentEntry != app.sectionInnerReelEntry.length - 1) {
                            TweenMax.set(app.sectionInner, {
                                overflowX: 'hidden',
                                scrollLeft: 0
                            });
                            ...
                            TweenMax.set(app.sectionInnerReel, {
                                x: `-${app.sectionInnerReelDistanceFromOrigin}`
                            });
                            TweenMax.to(app.sectionInnerReel, 0.5, {
                                x: app.reelEntryPositions[app.reelDragDetails.currentEntry + 1].x,
                                ease: Expo.easeOut,
                                onComplete: function () {
                                    ...
                                    TweenMax.set(app.sectionInner, {
                                        scrollLeft: Math.abs(app.reelEntryPositions[app.reelDragDetails.currentEntry + 1].x)
                                    });
                                    TweenMax.set(app.sectionInnerReel, {
                                        pointerEvents: 'auto'
                                    });
                                    ...
                                    app.swipeInProgress = false;
                                }
                            }, 0);
                        }
                    }

                    if (app.reelDragDetails.currentDragDirection === 'right') {
                        blockMenuHeaderScroll = true;
                        if (app.reelDragDetails.currentEntry != 0) {
                            TweenMax.set(app.sectionInner, {
                                overflowX: 'hidden',
                                scrollLeft: 0
                            });
