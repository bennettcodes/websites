<?php 
 define('SES_Server', 'email-smtp.us-east-1.amazonaws.com');
  define('SES_Port', 465);
  define('SES_AuthType', 'ssl');
  define('SES_UserName', 'TEST');
  define('SES_PassWord', 'TEST');
  define('SALT', 'goodsalt');
  
  //Info Email
  define('INFO_EMAIL','fer@fer-cpe.com');  
  define('SWEEPSSTAKES_EMAIL','sweepstakes@icdccollege.edu');
 
 include '../../application/swift/swift_required.php';   
 include '../../model/mail.class.php';
...
/*	-------------------------------------------------------------------------
	...
	if ($errors) {
		...
		if($recipients == true){
			$recipients = array(
				"fer@fer-cpe.com" => "FER CPE",
				"bwaisbren@infocusadv.com" => "Bennett Waisbren",
				"adejoya@infocusadv.com" => "Amanda De Joya",
				"rhenderson@infocusadv.com" => "Rachelle Henderson",
			);
		...
		$n = $fermail->SendMailAll(array(), $recipients, $mail->From, $mail->FromName, $mail->Subject, $mail->AltBody, $mail->Body, NULL);
